...
define("VERSION", date('Y-m-d').'http://www.feinian.com.cn');
$sqlFile = 'data.sql';
...
switch ($step) {
    ...
    case '4':
        ...
        if (intval($_GET['install'])) {
            ...
            $dbPrefix = empty($_POST['dbprefix']) ? 'fn_' : trim($_POST['dbprefix']);
            ...
            $sqldata = str_replace('temp_',$dbPrefix,$sqldata);
            ...
            //插入管理员
            $dbPrefixadmin = $dbPrefix.'rbac_admin';
            $salt = randCode(4);
            $password = regEncry($password, $salt);
            $query = "update `$dbPrefixadmin`  set username='$username', password='$password', salt='$salt' where id='1'";
            mysql_query($query);
        }
...
function regEncry($password, $salt) {
    $encry =  md5(md5($password, $salt).'I~@j');;
    return $encry;
}
