        ...
        <form id="datares" method="post" class="form-horizontal">
            ...
            <div class="form-group">
                ...
                <div class="col-sm-10">
                    ...
                    <img width="200" height="200" src="#" id="imghead"  style="display:none"/>
                </div>
            </div>
            <div class="form-group">
                <label for="inputPassword3" class="col-sm-2 control-label">样张（多张图片）：</label>
                <div class="col-sm-10">
                    <section class="add_session">
                        <input id="multi_file_upload" name="multi_file_upload" type="file" multiple="true">
                        <div style="display:flex;flex-wrap:wrap" id="img_lists">
                        </div>
                    </section>
                </div>
            </div>
...
<script src="/asset/adminasset/multi/js/imgUp.js"></script>
    ...
    $(function() {
        $('#file_upload').uploadify({
            ...
            'onUploadSuccess' : function(file, data, response){
                 $('#headimg').val(data);
                 $('#imghead').show().attr('src',data);
            }
        });
    });
    function add_img(data) {
        var $icon = ("<div style='margin-left:10px;' class='img_item_div'>");
        $icon += "<input type='hidden' name='yangzhang[]'  value="+data+">";
        $icon += "<img width='200' height='200' src="+data+"  />";
        $icon += '<div class=zhezhao><a class=\"pull-right delete-pic\"><img src=\"/public/asset/close_u.png\"/>"+"</a></div></div>';
        $('#img_lists').append($icon);
        $(".img_item_div").hover(function(){
            $(this).children(".zhezhao").fadeIn();
        },function(){
            $(this).children(".zhezhao").fadeOut();
        })
        $(".img_item_div").on("click",".delete-pic",function(){
            $parents=$(this).parents(".img_item_div");
            $parents.remove();
        });
    }

    $(function() {
        $('#multi_file_upload').uploadify({
            'fileSizeLimit' : '1MB',
            'fileTypeExts' : '*.gif; *.jpg; *.png',
            'formData'     : {
                'timestamp' : '<?php echo $timestamp;?>',
    ...
    $('#save').bind('click', function(){
        ...
        $.ajax({
            ...
            success:function(data){//ajax返回的数据
                if (data.status == 'true') {
                    layer.msg('添加成功');
                    parent.$('#ArbetTable').bootstrapTable('refresh', '');
